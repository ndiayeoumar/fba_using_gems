---
title: "Compass data analysis Unnormalized"
...
-   with $\lambda$, smoothing parameter, set to 0.25 and the number of nearest neighbors set to its default value of 30;
...
# gene_exp_tsv <- read.delim(
#     file = file.path(user_data_directory, "linear_gene_expression_matrix.tsv"), 
#     header = TRUE)
# 
# gene_exp_tsv$MGI.symbol <- toupper(gene_exp_tsv$MGI.symbol)
# 
# write.table(gene_exp_tsv, 
#             file = file.path(user_data_directory, "linear_gene_expression_matrix.tsv"), 
#             row.names = FALSE, sep = "\t")
```
...
```{r}
if(!file.exists("../../data/eae/eae_cd4_compass_data_mgi_unnormalized.rds")){
    compass_data <- CompassData$new(settings = settings)
    saveRDS(compass_data, 
        file = "../../data/eae/eae_cd4_compass_data_mgi_unnormalized.rds")
} else compass_data <- readRDS("../../data/eae/eae_cd4_compass_data_mgi_unnormalized.rds")
```
...
In Figure \@ref(fig:rumapbystage) promiscous groups of cells are clearly visible. Each group contains cells of both stages. At first sight we can say that the first two UMAP dimensions don't separate cells based on their stage.

```{r rumapbystage, fig.cap="Each dot represents a single cell coloured based on their stage (Chronic or Onset)."}
ggplot(cell_info_with_umap_components, aes(x = component_1, 
...
In figure \@ref(fig:rUMAPbycelltype) we can't see group of cells containing only one single cell type. Thus, the first two UMAP dimensions seem not to separate cells based on their stage.

```{r rUMAPbycelltype, fig.cap="Each dot represents a single cell coloured based on their cell type."}
ggplot(cell_info_with_umap_components,
...
In Figure \@ref(fig:geneUMAPRNA)), the cells are UMAP-ordinated by expression levels and colored by the membership we previously calculated using the reaction consistencies (panel a). By eye, we only obtain 2 distinguishable clusters and no patterns seem to be visible regarding the cluster membership based on the reaction consistencies. In the other panels, the stage (b), cell type (c), and metabolic activity (d) are represented.
...
```{r figSubsystem, fig.width=10, fig.height=6, fig.cap="Number of significant reactions by subsystem, stratified by core and not core reactions."}
...
```{r figSubsystemES, fig.width=10, fig.height=6, fig.cap="Cohen's D values for significant reactions by subsystem, stratified by core and not core reactions, ordered by median value."}
ggplot(data = sig_subsystem, 
    ...
    scale_color_discrete(name = "More consistent in:", labels = c("Cluster 1", "All the other clusters"))
...
plot_metareaction(
    met_id = c("group_420","group_55", "group_224"),
...
```{r figSubsystem6vsall, fig.width=10, fig.height=6, fig.cap="Number of significant reactions by subsystem, stratified by core and not core reactions."}
