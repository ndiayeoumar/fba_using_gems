...
subtitle: "An example of metabolic profiles anlysis of Neutrophil cells in EAE based on meta-reactions"
author: 
    - Matteo Calgaro
date: "`r Sys.Date()`"
output: 
...
Note that the division into meta-reactions is data-driven and does not rely on canonical metabolic pathway definitions. Therefore, the division is dataset-dependent — for example, two reactions might be closely correlated and clustered in the same meta-reaction in one cell type, but not in another.
...
```{r}
user_data_directory <- "../../data/eae/neutrophils/tpm_counts_normalized"
...
```{r}
if(!file.exists("../../data/eae/neutrophils/eae_neutrophils_compass_data_mgi.rds")){
    compass_data <- CompassData$new(settings = settings)
    saveRDS(compass_data, 
        file = "../../data/eae/neutrophils/eae_neutrophils_compass_data_mgi.rds")
} else compass_data <- readRDS("../../data/eae/neutrophils/eae_neutrophils_compass_data_mgi.rds")
```
...
```{r}
compass_data$cell_metadata$cell_id <- make.names(compass_data$cell_metadata$Cell.name, unique = TRUE)
index_eae <- which(compass_data$cell_metadata$Condition == "EAE")
compass_data$cell_metadata <- compass_data$cell_metadata[index_eae, ]

compass_data$gene_expression_statistics <- compass_data$gene_expression_statistics[index_eae, ]

colnames(compass_data$metareaction_consistencies) <- make.names(colnames(compass_data$metareaction_consistencies), unique = TRUE)
colnames(compass_data$reaction_consistencies) <- make.names(colnames(compass_data$reaction_consistencies), unique = TRUE)

compass_data$reaction_consistencies <- compass_data$reaction_consistencies[, index_eae]

compass_data$metareaction_consistencies <- compass_data$metareaction_consistencies[, index_eae]
```
...
In Figure \@ref(fig:rumappmnclusters), cells are colored according to their cluster membership (PMN).

```{r rumappmnclusters, fig.cap="Each dot represents a single cell coloured based on their PMN cluster membership."}
# umap according to metabolic activity
p_m_umap_pmn <- ggplot(cell_info_with_dimensionality_reduction,
    aes(x = component_1, y = component_2, color = PMN.clusters)) +
    geom_point(alpha = 0.8) +
    theme(legend.position = "bottom") +
    ggtitle("UMAP (based on reaction scores) coloured by PMN cluster membership") +
    xlab(label ="UMAP 1") +
    ylab(label = "UMAP 2") +
...
In Figure \@ref(fig:rpcabymetabolicactivity), we can see a barely visible group of cells with higher metabolic activity in the lower-right part of the plot.
...
In Figure \@ref(fig:rpcapmnclusters), cells are colored according to their cluster membership (PMN).

```{r rpcapmnclusters, fig.cap="Each dot represents a single cell coloured based on their PMN cluster membership."}
# umap according to metabolic activity
p_m_pca_pmn <- ggplot(cell_info_with_dimensionality_reduction,
    aes(x = PCA_1, y = PCA_2, color = PMN.clusters)) +
    geom_point(alpha = 0.8) +
    theme(legend.position = "bottom") +
    ggtitle("PCA (based on reaction scores) coloured by PMN cluster membership") +
    xlab(label ="PCA 1") +
    ylab(label = "PCA 2") +
    scale_color_manual(values = RColorBrewer::brewer.pal(5, "Set1")) +
    theme(legend.position = "bottom")
...
To automatically detect the number of clusters, we will use the function `get_optimal_clusters()` to identify the number of clusters which maximizes the silhouette index.
...
opt_cl <- get_optimal_clusters(
  UMAPcoords = cell_info_with_dimensionality_reduction[, c("component_1", "component_2", "component_3", "component_4", "component_5")], method = "average",
  k.values = 2:15,
  plotIt = TRUE)
```

The optimal number of clusters is 3. To obtain the cluster membership for each cell, the `function get_cluster()` can be used. The number of cells in each cluster, stratified by cell subset and stage is summarized in Figure \@ref(fig:plotCellTypeCluster).
...
```{r}
gene_exp_umap_comps <- read.delim(file = "../../data/eae/neutrophils/Neutrophils_EAE_batched_UMAP_coordinates.csv")
gene_exp_umap_comps <- data.frame("UMAP1" = t(gene_exp_umap_comps[1,-1])[index_eae], 
                                  "UMAP2" = t(gene_exp_umap_comps[2,-1])[index_eae],
                                  "cell_id" = (colnames(gene_exp_umap_comps)[-1])[index_eae])

gene_exp_pca_comps <- read.delim(file = "../../data/eae/neutrophils/Neutrophils_EAE_batched_PCA_coordinates.csv")
gene_exp_pca_comps <- data.frame("PCA1" = t(gene_exp_pca_comps[1,-1])[index_eae], 
                                  "PCA2" = t(gene_exp_pca_comps[2,-1])[index_eae],
                                  "cell_id" = (colnames(gene_exp_pca_comps)[-1])[index_eae])
...
In Figure \@ref(fig:geneUMAPRNA)), the cells are UMAP-ordinated by expression levels and coloured by the membership we previously calculated using the metareaction consistencies (panel a). By eye, no patterns seem to be visible regarding the cluster membership based on the metareaction consistencies (maybe a slightly increased abundance of oranges on the top). In the other panels, the Stage (b), PMN cluster membership (c), and metabolic activity (d) are represented.

```{r geneUMAPRNA, fig.cap="Each dot represents a single cell coloured by cluster membership computed from the metareaction consistencies (a), Stage (b), PMN cluster membership (c), and metabolic activity (d). The UMAPs are based on the gene expression values.", fig.width=14, fig.height=16}
...
p_t_umap_pnm <- ggplot(cell_info_with_dimensionality_reduction,
    aes(x = UMAP1, y = UMAP2, color = as.factor(PMN.clusters))) +
    geom_point(alpha = 0.8) +
    ggtitle("UMAP (based on gene expression) coloured by PMN cluster") +
    xlab(label = "UMAP 1") +
    ylab(label = "UMAP 2") +
    scale_color_manual(values = RColorBrewer::brewer.pal(5, "Set1")) +
    theme(legend.position = "bottom") +
    guides(color = guide_legend(title = "PMN cluster"))
...
cowplot::plot_grid(p_t_umap_cluster, p_t_umap_stage, p_t_umap_pnm, p_t_umap_ma, nrow = 2, align = "hv", axis = "tblr", labels = "auto")

```

## UMAP - Metabolic profile

```{r geneUMAPmetabolic, fig.cap="Each dot represents a single cell coloured by cluster membership computed from the metareaction consistencies (a), stage (b), cell subset (c), and metabolic activity (d). The UMAPs are based on the gene expression values.", fig.width=14, fig.height=16}
cowplot::plot_grid(p_m_umap_cluster, p_m_umap_stage, p_m_umap_pmn, p_m_umap_ma, nrow = 2, align = "hv", axis = "tblr", labels = "auto")
```

## PCA - Transcriptional profile

In Figure \@ref(fig:genePCARNA)), the cells are PCA-ordinated by expression levels and coloured by the membership we previously calculated using the metareaction consistencies (panel a). By eye, no patterns seem to be visible regarding the cluster membership based on the metareaction consistencies. In the other panels, the stage (b), cell subset (c), and metabolic activity (d) are represented.

```{r genePCARNA, fig.cap="Each dot represents a single cell coloured by cluster membership computed from the metareaction consistencies (a), stage (b), cell subset (c), and metabolic activity (d). The PCs are based on the gene expression values.", fig.width=14, fig.height=16}
...
p_t_pca_pmn <- ggplot(cell_info_with_dimensionality_reduction,
    aes(x = PCA1, y = PCA2, color = PMN.clusters)) +
    geom_point(alpha = 0.8) +
    ggtitle("PCA (based on gene expression) coloured by PMN cluster membership") +
    xlab(label = "PCA 1") +
    ylab(label = "PCA 2") +
...
cowplot::plot_grid(p_t_pca_cluster, p_t_pca_stage, p_t_pca_pmn, p_t_pca_ma, nrow = 2, align = "hv", axis = "tblr", labels = "auto")
```

## PCA - Metabolic profile

```{r genePCAmetabolic, fig.cap="Each dot represents a single cell coloured by cluster membership computed from the metareaction consistencies (a), Stage (b), PMN cluster membership (c), and metabolic activity (d). The PCAs are based on the gene expression values.", fig.width=14, fig.height=16}
cowplot::plot_grid(p_m_pca_cluster, p_m_pca_stage, p_m_pca_pmn, p_m_pca_ma, nrow = 2, align = "hv", axis = "tblr", labels = "auto")
```

## Focus - Chronic vs Onset

``` {r genePCAUMAP, fig.cap="PCA (a) and UMAP (b) applied to gene expression. Each dot represents a cell coloured by stage.", fig.width=10, fig.height=5}
cowplot::plot_grid(p_t_pca_stage, p_t_umap_stage, nrow = 1, align = "hv", axis = "tblr", labels = "auto")
```
Figure \@ref(fig:genePCAUMAP) summarises both UMAP and PCA applied on the transcriptomic data coloured by stage (chronic vs onset). None of the approaches creates separated clusters.

``` {r metabolicPCAUMAP, fig.cap="PCA (a) and UMAP (b) applied to metabolic reaction scores computed by Compass. Each dot represents a cell coloured by stage", fig.width=10, fig.height=5}
cowplot::plot_grid(p_m_pca_stage, p_m_umap_stage, nrow = 1, align = "hv", axis = "tblr", labels = "auto")
```
Figure \@ref(fig:metabolicPCAUMAP) summarises both UMAP and PCA applied on the metareaction scores coloured by stage (chronic vs onset). More clearly defined clusters become apparent, particularly with UMAP. This implies that these data might provide more valuable insights compared to transcriptomic data. Nevertheless, it was intriguing to observe the lack of clear cell segregation based on their inflammatory status, contrary to what was anticipated. This prompted us to conduct a more in-depth investigation to comprehend the distinct features that played a role in the development of these segregated groups.
...
write.csv(x = 
    cell_info_with_dimensionality_reduction[, 
        c("cell_id", "cluster")], 
    file = "../../data/eae/neutrophils/cluster_annotation.csv", row.names = FALSE)
```
...
```{r plotCellTypeCluster, fig.cap="Cells distribution across stages and clusters.", fig.height=6, fig.width=4}
# Count how many cells for each cluster, stage, PMN cluster
df_cluster <- plyr::ddply(.data = cell_info_with_dimensionality_reduction, 
    .variables = ~ cluster + Stage + PMN.clusters, 
...
main_plot <- ggplot(data = df_cluster, 
    aes(x = cluster, y = PMN.clusters)) +
    # coord_equal() + 
    geom_tile(aes(fill = PMN.clusters, alpha = n), width = 0.8, height = 0.8) +
...
# Count how many cells for each cluster, stage
df_cluster <- plyr::ddply(.data = cell_info_with_dimensionality_reduction, 
...
```{r}
cell_ids <- paste0("Cell",
    1:nrow(cell_info_with_dimensionality_reduction))
cell_info_with_dimensionality_reduction$cell_id <- cell_ids
colnames(compass_data$metareaction_consistencies) <- cell_ids

meta_1_all <- compassR::compare_clusters(
...
The effect size is summarized by the Cohen's D statistic, which is computed as the difference between the mean of the two groups, divided by the pooled standard deviation. Positive values represent reactions which are more consistent in Cluster 1, while negative values represent reactions that are more consistent in all the other clusters. To summarize the results, several plots are proposed.
...
```{r figVolcano1VsAll, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
...
```{r}
meta_2_all <- compassR::compare_clusters(
    cell_info = cell_info_with_dimensionality_reduction,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "cluster",
...
The effect size is summarized by the Cohen's D statistic, which is computed as the difference between the mean of the two groups, divided by the pooled standard deviation. Positive values represent reactions which are more consistent in Cluster 2, while negative values represent reactions that are more consistent in all the other clusters. To summarize the results, several plots are proposed.
...
```{r figVolcano2VsAll, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
...
```{r}
meta_3_all <- compassR::compare_clusters(
    cell_info = cell_info_with_dimensionality_reduction,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "cluster",
...
```{r fig3VsAll, fig.width=15, fig.height=6, fig.cap="Significant reactions by subsystem."}
cowplot::plot_grid(
    meta_3_all$p_DC_reactions,
    meta_3_all$p_DC_reactions_cohensd + 
        scale_color_manual(values = scales::hue_pal()(2)), labels = "auto") 
```
...
```{r figVolcano3VsAll, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
...
```{r figSumOneVsAll, fig.cap="Summary of all one-vs-all cluster comparisons. The signed logarithm of differences between the number of differentially consistent reactions between the two cluster groups is used to color the heatmap. A blueish shade means that the reactions are more consistent on the second group of cells, a redish shade means that the reactions are more consistent on the first group of cells.", fig.width=7, fig.height=10}
# Collect all tables
...
pheatmap(mat,
         ...
         scale = "none",
         cluster_cols = FALSE, 
         main = "One vs All clusters comparisons
         \n Summary heatmap",
         breaks = my_breaks)
```
...
```{r}
meta_1_2 <- compassR::compare_clusters(
    cell_info = cell_info_with_dimensionality_reduction,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "cluster",
...
The effect size is summarized by the Cohen's D statistic, which is computed as the difference between the mean of the two groups, divided by the pooled standard deviation. Positive values represent reactions which are more consistent in Cluster 1, while negative values represent reactions that are more consistent in cluster 2. To summarize the results, several plots are proposed.
...
```{r figVolcano1Vs2, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
...
```{r}
meta_1_3 <- compassR::compare_clusters(
    cell_info = cell_info_with_dimensionality_reduction,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "cluster",
...
```{r fig1Vs3, fig.width=15, fig.height=7, fig.cap="Significant reactions by subsystem."}
cowplot::plot_grid(meta_1_3$p_DC_reactions, meta_1_3$p_DC_reactions_cohensd, labels = "auto") 
```

The effect size is summarized by the Cohen's D statistic, which is computed as the difference between the mean of the two groups, divided by the pooled standard deviation. Positive values represent reactions which are more consistent in Cluster 1, while negative values represent reactions that are more consistent in cluster 3. To summarize the results, several plots are proposed. 
...
```{r figVolcano1Vs3, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
...
```{r}
meta_2_3 <- compassR::compare_clusters(
    cell_info = cell_info_with_dimensionality_reduction,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "cluster",
...
The effect size is summarized by the Cohen's D statistic, which is computed as the difference between the mean of the two groups, divided by the pooled standard deviation. Positive values represent reactions which are more consistent in Cluster 2, while negative values represent reactions that are more consistent in cluster 3. To summarize the results, several plots are proposed. 
...
```{r figVolcano2Vs3, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
meta_2_3_results$plot
```

### Summary image

```{r figSumOneVsOne, fig.cap="Summary of all one-vs-one cluster comparisons. The signed logarithm of differences between the number of differentially consistent reactions between the two cluster groups is used to color the heatmap. A blueish shade means that the reactions are more consistent on the second group of cells, a redish shade means that the reactions are more consistent on the first group of cells.", fig.width=6, fig.height=12}
# Collect all tables
...
pheatmap(mat,
         ...
         scale = "none",
         cluster_cols = FALSE, 
         main = "Paiwise cluster comparisons \n Summary heatmap",
         breaks = my_breaks)
```
...
Let's inspect the subsystems of interest, the following table and Figure \@ref(fig:figVolcano1stage) are presented.
...
DT::datatable(meta_1_stages_results$faceted_results[, c(
  "subsystem_priority", "subsystem",
    "reaction_name", "reaction_id",
    "cohens_d", "adjusted_p_value")] %>%
      dplyr::filter(adjusted_p_value < 0.05),
...
```{r figVolcano1stage, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
meta_1_stages_results$plot
```
...
```{r, warning=FALSE}
meta_2_stages <- compassR::compare_clusters(
    cell_info = cluster_2,
    compass_analyzer = compass_analyzer,
    compass_data = compass_data,
    variable = "Stage",
...
meta_2_stages_results <- get_faceted_results(
    ...
    facets = facets)
DT::datatable(meta_2_stages_results$faceted_results[, c(
  "subsystem_priority", "subsystem",
    "reaction_name", "reaction_id",
    "cohens_d", "adjusted_p_value")] %>%
      dplyr::filter(adjusted_p_value < 0.05),
...
```{r figVolcano2stage, fig.cap="Volcano plot for the reactions belonging to significant meta-reactions colored by subsistem of interest.", fig.height=5, fig.width=10}
meta_2_stages_results$plot
```
### Cluster 3
...
No differences (very few chronic cells).

### Summary image

```{r figSumStage, fig.cap="Summary of all Chronic vs Onset within cluster comparisons. The signed logarithm of differences between the number of differentially consistent reactions between the two cluster groups is used to color the heatmap. A blueish shade means that the reactions are more consistent on the second group of cells, a redish shade means that the reactions are more consistent on the first group of cells.", fig.width=6, fig.height=12}
# Collect all tables
...
pheatmap(mat,
         ...
         scale = "none",
         cluster_cols = FALSE, 
         main = "Chronic vs Onset inside cluster \n Summary heatmap",
         breaks = my_breaks)
```

```{r stageUMAP12, fig.cap="Each dot represents a single cell coloured by stage. (a) has UMAP for cells of cluster 1, while (b) contains cells of cluster 2. The UMAP components are based on the metareaction consistencies.", fig.width=10, fig.height=5}
cowplot::plot_grid(ellipse1_stage, ellipse2_stage, nrow = 1, align = "hv", axis = "tblr", labels = "auto")
```